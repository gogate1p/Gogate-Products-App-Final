var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
import { Body, Controller, Get, Param, Post } from '@nestjs/common';
import { PackageScanningService } from './package-scanning.service.js';
let PackagesController = class PackagesController {
    packageScanningService;
    constructor(packageScanningService) {
        this.packageScanningService = packageScanningService;
    }
    async getPackage(id, tenantId) {
        return this.packageScanningService.getPackageById(id, tenantId);
    }
    async getPackageScans(id, tenantId) {
        return this.packageScanningService.getPackageHistory(id, tenantId);
    }
    async scanPackage(request) {
        return this.packageScanningService.scanPackage(request);
    }
    async scanBatch(requests) {
        return this.packageScanningService.scanBatch(requests ?? []);
    }
};
__decorate([
    Get(':id'),
    __param(0, Param('id')),
    __param(1, Body('tenantId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], PackagesController.prototype, "getPackage", null);
__decorate([
    Get(':id/scans'),
    __param(0, Param('id')),
    __param(1, Body('tenantId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], PackagesController.prototype, "getPackageScans", null);
__decorate([
    Post('scan'),
    __param(0, Body()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], PackagesController.prototype, "scanPackage", null);
__decorate([
    Post('scan/batch'),
    __param(0, Body('requests')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Array]),
    __metadata("design:returntype", Promise)
], PackagesController.prototype, "scanBatch", null);
PackagesController = __decorate([
    Controller('packages'),
    __metadata("design:paramtypes", [PackageScanningService])
], PackagesController);
export { PackagesController };
//# sourceMappingURL=packages.controller.js.map